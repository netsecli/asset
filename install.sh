...
{
	if [ -f elk.tar ];then
		docker image load -i elk.tar &> /dev/null
    fi
	...
	if [ $? -eq 0 ];then
#		echo "/usr/bin/python2.7 nmap_es.py -i /opt/nmap-xml/nmap-${Date}.xml -e 127.0.0.1 -r nmap -I nmap-${Date} -u ${elastic_user} -P ${elastic_pass}"
